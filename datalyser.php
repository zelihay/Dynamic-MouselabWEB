...
//
//       v 1.00b3, Sep 13 2010
//
//	   may 15 updated version with larger memory limit (v2)
//		sept 13 updated with full php tags (v3)
//
//     (c) 2004-08 Martijn C. Willemsen and Eric J. Johnson 
//
...
function ExpfromDB($nameofexp,$table)
{
    ...
    # Execute SQL query 
    $result=mysqli_query($conn, $sqlQuery) or die("Invalid Query2 : ".mysqli_error($conn)); 
   		...
   		{ 
	        ...
	        if(!($i%2)) 
			{
	    	    ...
	    	    if($key=="adddata")
				{
					...
					//$newdataarr=explode(";",eregi_replace("\"","",$value));
					$newdataarr=explode(";", $val); //php7
					$last = array_pop($newdataarr);
				}
				else if ($key=="addvar")
				{
					$newvararr=explode(";",$value);
					$last = array_pop($newvararr);
				}
				else 
				{
					$rowarr[$j][]=$value;
...
<?php
    ...
    # Execute SQL query 
    $result=mysqli_query($conn, $sqlQuery) or die("Invalid Query3 : ".mysqli_error($conn)); 
...
$sqlQuery = "SELECT DISTINCT expname from $table";
    ...
    # Execute SQL query 
    $result=mysqli_query($conn, $sqlQuery) or die("Invalid Query4 : ".mysqli_error($conn)); 
...
<?php
    ...
    { 
	    ...
	    # Execute SQL query 
   		$res2=mysqli_query($conn, $sqlQuery) or die("Invalid Query1 : ".mysqli_error($conn)); 
